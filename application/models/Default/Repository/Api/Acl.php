 ...
 * @author Renato David
 */
class Application_Model_Default_Repository_Api_Acl extends Sky_Db_Repository_Abstract {
    ...
    public function insertRule($privilege,$role,$allow=null){
        ...
        if(is_array($privilege)) {
            ...
            foreach ($privilege as $pr) {
                ...
                foreach ($pr as $prv){
                    ...
                    if(count($row)>0){
                        $id[] = $rule->insert(array('privilege_id'=>$row['privilege_id'],'role_id'=>$role,'allow'=>$allow,'resource_name'=>$row['module_name'] . ":" . $row['class_name']));
                    }
                }
            }
        ...
        if(count($row)>0)
            return $rule->insert(array('privilege_id'=>$row['privilege_id'],'role_id'=>$role,'allow'=>$allow,'resource_name'=>$row['module_name'] . ":" . $row['class_name']));
    ...
    public function getPrivileges($where=null){
        ...
        foreach ($rows as $row) {
            $values[$row['module_name']][$row['class_name']][$row['name']] = $row['description'];
        }
