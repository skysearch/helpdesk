 ...
 */
class Api_Services_Acl extends Zend_Acl {
...
//		ini_set('max_execution_time', 120);
    ...
    private function _buildResources() {
        ...
        foreach ($resources as $row) {
            ...
            $map[$row['resource_id']] = $row['module_name'] . ':' . $row['class_name'];
        }
        ...
        while ($numResources > $i) {
            foreach ($resources as $row) {
                 ...
                 */
                $resId = $row['module_name'] . ':' . $row['class_name'];
    ...
    private function _buildRoles() {
        ...
        $roles = Sky_Model_Factory::getInstance()->setModule('api')->getAcl()->getRoles();
    ...
    private function _buildRules() {
        $rules = Sky_Model_Factory::getInstance()->setModule('api')->getAcl()->getRules();
